...
@interface RoomController() <TKEduBoardDelegate,TKEduSessionDelegate,UIGestureRecognizerDelegate,UIScrollViewDelegate,UITableViewDelegate,UITableViewDataSource,CAAnimationDelegate,UIImagePickerControllerDelegate,TKEduNetWorkDelegate,UINavigationControllerDelegate>
{
    
    CGFloat _maxVideo;
    CGFloat _sBottomViewHeigh;
    CGFloat _sBottomViewWidth;
    CGFloat _sStudentVideoViewHeigh;
    CGFloat _sStudentVideoViewWidth;
    CGFloat _sTeacherVideoWidth;//老师视频宽度
    CGFloat _sTeacherVideoHeight;
...
@property (nonatomic, strong) TKNewNavView *titleView;//导航栏
...
-(void)fullScreenToLc:(NSNotification*)aNotification{
    ...
    if (isFull) {
        ...
        [self.backgroundImageView sendSubviewToBack:_iTKEduWhiteBoardView];
        [self refreshUI];
    }
...
- (void)viewDidLoad {
    ...
    _sTeacherVideoWidth = _sStudentVideoViewWidth * 1.6;
    ...
    _sTeacherVideoHeight = _sTeacherVideoWidth*dpi;
    
    _sBottomViewWidth = [TKUtil isiPhoneX]?ScreenW-(_sTeacherVideoWidth+2*sViewCap * Proportion)-60:ScreenW-(_sTeacherVideoWidth+2*sViewCap * Proportion);
    
    _sStudentVideoViewWidth =(_sBottomViewWidth-VideoSmallViewMargins*7)/6.0;
    
    _sStudentVideoViewHeigh =_sStudentVideoViewWidth*dpi;
    
    _sBottomViewHeigh = _sStudentVideoViewHeigh+2*VideoSmallViewMargins;
    ...
    [self createTimer];
    [self.backgroundImageView bringSubviewToFront:_iTKEduWhiteBoardView];
    [self.backgroundImageView bringSubviewToFront:_iClassTimeView];
    //todo
...
#pragma mark - 导航栏
-(void)initNavigationNew {
    
    self.navigationController.navigationBar.hidden = YES;
    
    self.titleView = [[TKNewNavView alloc]initWithFrame:CGRectMake(0, 0, ScreenW, self.navbarHeight) aParamDic:_iParamDic roomType:_iRoomType];
    [self.backgroundImageView addSubview:self.titleView];
    //    [self.view addSubview:self.titleView];
    __weak RoomController * roomVC = self;
    
    self.titleView.leaveButtonBlock = ^{//离开课堂 （返回)
        
        [roomVC leftButtonPress];
        
    };
    
    self.titleView.showCoursewareViewBlock = ^(BOOL isSelected) {//课件库按钮
        ...
        if (isSelected) {
            if (!roomVC.listView) {
                ...
                if (roomVC.userListView) {
                    ...
                    roomVC.titleView.memberButton.selected = NO;
                    roomVC.userListView = nil;
                }
                
                if (roomVC.controlView) {
                    [roomVC.controlView hidden];
                    roomVC.titleView.controlButton.selected = NO;
                ...
                //文件列表：            宽 7/10  高 9/10
                CGFloat showHeight = roomVC.iTKEduWhiteBoardView.height*(7/10.0);
                ...
                roomVC.listView = [[TKListView alloc]initWithFrame:CGRectMake(x,y, showWidth, showHeight) andTitle:@"dd" from:@"RoomController"];
                
                roomVC.listView.dismissBlock = ^{
                    roomVC.titleView.coursewareButton.selected = NO;
                    roomVC.listView = nil;
                };
    ...
    self.titleView.showMemberViewBlock = ^(BOOL isSelected) {//成员
        if (isSelected) {
            
            roomVC.titleView.showRedDot = NO;
            ...
            if (!roomVC.userListView) {
                ...
                CGFloat showWidth = roomVC.iTKEduWhiteBoardView.width * (8/10.0);
                CGFloat x = (roomVC.iTKEduWhiteBoardView.width-showWidth)/2.0;
                CGFloat y = (ScreenH-_navbarHeight-_sBottomViewHeigh)*(1/10.0);
                
                
                roomVC.userListView = [[TKUserListView alloc]initWithFrame:CGRectMake(x,y,showWidth,showHeight) userList:@"RoomController"];
    ...
    self.titleView.showControlViewBlock = ^(BOOL isSelected) {//控制视图
        if (isSelected) {
            if (!roomVC.controlView) {
                ...
                CGFloat showHeight = roomVC.iTKEduWhiteBoardView.height*(6/10.0);
                CGFloat showWidth = roomVC.iTKEduWhiteBoardView.width * (6/10.0);
                CGFloat x = (roomVC.iTKEduWhiteBoardView.width-showWidth)/2.0;
                CGFloat y = (roomVC.iTKEduWhiteBoardView.height-showHeight)/2.0;
                
                roomVC.controlView = [[TKControlView alloc]initWithFrame:CGRectMake(x,y,showWidth,showHeight) controlController:@"RoomController"];
                roomVC.controlView.resetBlock = ^{//全部恢复
                    ...
                    for (TKOldVideoSmallView *view in sArray) {
                    ...
                    //将拖拽的视频还原
                    for (TKOldVideoSmallView *view  in roomVC.iStudentVideoViewArray) {
    ...
    self.titleView.classBeginBlock = ^{//上课
        
        [roomVC hiddenNavAlertView];
    };
    
    
    self.titleView.classoverBlock = ^{//下课
    ...
    // 相册
    self.titleView.showPictureViewBlock = ^(BOOL isSelected) {
        if (isSelected) {
            roomVC.titleView.toolsButton.selected = NO;
            [roomVC chooseAction:0 delay:NO];
        }
    };
    
    
    // 相机
    self.titleView.showCamareViewBlock = ^(BOOL isSelected) {
        if (isSelected) {
            roomVC.titleView.toolsButton.selected = NO;
            [roomVC chooseAction:1 delay:NO];
        }
    };
    
    
    self.titleView.showToolsViewBlock = ^(BOOL isSelected) {
        if (isSelected) {
            if (roomVC.listView) {
                [roomVC.listView hidden];
                roomVC.titleView.coursewareButton.selected = NO;
...
- (void)hiddenNavAlertView{
    if (self.listView) {
        ...
        self.titleView.coursewareButton.selected = NO;
        self.listView = nil;
    }
    
    if (self.userListView) {
        [self.userListView hidden];
        self.titleView.memberButton.selected = NO;
        self.userListView = nil;
    }
    
    if (self.controlView) {
        [self.controlView hidden];
        self.titleView.controlButton.selected = NO;
        self.controlView = nil;
    }
    
    // 隐藏工具箱
    [[TKEduSessionHandle shareInstance].whiteBoardManager showToolbox:NO];
    self.titleView.toolsButton.selected = NO;
}
#pragma mark 点击上课
- (void)handTouchDown{
...
- (void)refreshUI
{
    ...
    //老师
    CGFloat tViewWidth = _sTeacherVideoWidth;
    CGFloat tTeacherHeight = _sTeacherVideoHeight;
    {
...
-(void)initRightView{
    ...
    {//右视图
        ...
        CGFloat tRightW = _sTeacherVideoWidth + 2 * tViewCap;
        CGRect tRithtFrame = CGRectMake([TKUtil isiPhoneX]?ScreenW-30-tRightW:ScreenW-tRightW, tRightY, tRightW, [TKUtil isiPhoneX]?ScreenH-tRightY-17:ScreenH-tRightY);
        ...
        [self.backgroundImageView addSubview:_iRightView];
    ...
    //老师
    {
        ...
        _iTeacherVideoView= ({
            ...
            TKOldVideoSmallView *tTeacherVideoView = [[TKOldVideoSmallView alloc]initWithFrame:CGRectMake(tViewCap, tViewCap, tViewWidth, tTeacherHeight) aVideoRole:EVideoRoleTeacher];
        ...
        _iTeacherVideoView.oneKeyResetBlock = ^{//全部恢复
            ...
            //将拖拽的视频还原
            for (TKOldVideoSmallView *view  in weakSelf.iStudentVideoViewArray) {
    ...
    _iRightView.sakura.backgroundColor(ThemeKP(@"chatBackgroundColor"));
    
}

#pragma mark - 分屏/取消分屏
- (void)beginTKSplitScreenView:(TKOldVideoSmallView*)videoView{
    ...
    if (!videoView.isSplit) {
        ...
        for (TKOldVideoSmallView *view in videoArray) {
...
-(void)initWhiteBoardView{
    ...
    CGFloat tWidth =  [TKUtil isiPhoneX] ? ScreenW-60- self.iRightView.width:ScreenW - self.iRightView.width;
    CGFloat tHeight = ScreenH - CGRectGetMaxY(self.titleView.frame) - CGRectGetHeight(self.iBottomView.frame);
    
    CGFloat x = [TKUtil isiPhoneX]?30:0;
    
    CGRect tFrame = CGRectMake(x, CGRectGetMaxY(self.titleView.frame),tWidth, tHeight);
...
- (void)initSplitScreenView{
    ...
    CGRect tFrame = CGRectMake(0, 0, self.iTKEduWhiteBoardView.width,self.iTKEduWhiteBoardView.height);
...
-(void)refreshWhiteBoard:(BOOL)hasAnimate{
    ...
    CGFloat tWidth =  [TKUtil isiPhoneX]?ScreenW-60- self.iRightView.width:ScreenW- self.iRightView.width;
    CGFloat tHeight = [TKEduSessionHandle shareInstance].iHasPublishStd ?ScreenH - CGRectGetMaxY(self.titleView.frame) - CGRectGetHeight(self.iBottomView.frame):ScreenH - CGRectGetMaxY(self.titleView.frame);
    ...
    CGRect tFrame = CGRectMake(x, y, tWidth, tHeight);
...
-(void)initBottomView{
    ...
    _iBottomView = ({
        UIView *tBottomView = [[UIView alloc]initWithFrame:CGRectMake([TKUtil isiPhoneX]?30:0, [TKUtil isiPhoneX]?ScreenH-_sBottomViewHeigh-17:ScreenH-_sBottomViewHeigh, _sBottomViewWidth, _sBottomViewHeigh)];
        tBottomView;
    });
    
    _iBottomView.sakura.backgroundColor(@"ClassRoom.BottomView.backColor");
    
    [self.backgroundImageView addSubview:_iBottomView];
    ...
    for (NSInteger i = 0; i < _iRoomProperty.iMaxVideo.intValue-1; ++i) {
        ...
        // 判断当前视频窗口是否与白板相交
        __weak typeof(TKOldVideoSmallView *) wtOurVideoBottomView = tOurVideoBottomView;
    ...
    for (TKOldVideoSmallView *view in _iStudentVideoViewArray) {
        ...
        if (view.iRoomUser) {
            ...
            if (!view.isSplit && view.isDrag == NO) {//判断是否分屏
                ...
                view.alpha  = 1;
                view.frame = CGRectMake(left, tCap+CGRectGetMinY(_iBottomView.frame), tWidth, tHeight);
                left += tCap + tWidth;
...
- (void)initPlaybackMaskView {
    self.playbackMaskView = [[TKPlaybackMaskView alloc] initWithFrame:CGRectMake(0,
                                                                                 self.navbarHeight,
                                                                                 ScreenW,
                                                                                 ScreenH - self.navbarHeight)];
    //    :CGRectMake(0, self.navbarHeight, ScreenW, ScreenH- self.navbarHeight)];
...
-(void)playVideo:(TKRoomUser *)user {
    ...
    TKOldVideoSmallView* viewToSee = nil;
...
-(void)unPlayVideo:(NSString *)peerID {
    ...
    else
    {
        ...
        for (TKOldVideoSmallView* view in _splitScreenView.videoSmallViewArray) {
            if(view.iRoomUser != nil && [view.iRoomUser.peerID isEqualToString:peerID]) {
...
-(void)myUnPlayVideo:(NSString *)peerID aVideoView:(TKOldVideoSmallView*)aVideoView completion:(void (^)(NSError *error))completion{
...
-(void)myPlayVideo:(TKRoomUser *)aRoomUser aVideoView:(TKOldVideoSmallView*)aVideoView completion:(void (^)(NSError *error))completion{
...
- (void)boardOnViewStateUpdate:(NSDictionary *)message{
    [self.titleView updateView:message];
}
...
- (void)sessionManagerRoomJoined{
    ...
    _iRoomType       = (RoomType)[_iSessionHandle.roomMgr getRoomProperty].roomtype;
    ...
    [self.titleView setTitle:roomname];
...
- (void)sessionManagerRoomLeft {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver: self.chatView];
}
#pragma mark - 踢出课堂
...
- (void)sessionManagerOnAudioVolumeWithPeerID:(NSString *)peeID volume:(int)volume{
    ...
    NSDictionary *dict = @{@"volume":@(volume),@"peeID":peeID};
...
- (void)sessionManagerUserChanged:(TKRoomUser *)user Properties:(NSDictionary*)properties fromId:(NSString *)fromId {
    ...
    if ([properties objectForKey:sRaisehand]) {
        ...
        self.titleView.showRedDot = isRaiseHand;
    ...
    if (_iUserType == UserType_Student && [_iSessionHandle.localUser.peerID isEqualToString:user.peerID]) {
        ...
        if (isRaiseHand) {
            if (_iSessionHandle.localUser.publishState > 0) {
                [self.titleView setHandButtonState:YES];
            }
        } else {
            [self.titleView setHandButtonState:NO];
        }
...
- (void)sessionManagerDidFailWithError:(NSError *)error {
    ...
    if (!(error.code == TKErrorCode_ConnectSocketError || error.code == TKErrorCode_Subscribe_RoomNotExist || error.code == TKRoomWarning_ReConnectSocket_ServerChanged)) {
        return;
    }
...
- (void)clearAll
{
    ...
    if ([TKEduSessionHandle shareInstance].msgList) {
        [[TKEduSessionHandle shareInstance].msgList removeAllObjects];
    }
    ...
    for (TKOldVideoSmallView *view in array) {
...
- (void)clearVideoViewData:(TKOldVideoSmallView *)videoView {
    
    videoView.isDrag = NO;
    if (videoView.iRoomUser != nil) {
        //        if (![videoView.iRoomUser.peerID isEqualToString:_iSessionHandle.localUser.peerID]) {
...
//                }
            ...
            if (!_iSessionHandle.isPlayback && ![_iSessionHandle.roomMgr getRoomConfigration].beforeClassPubVideoFlag && _iSessionHandle.isPlayback == NO) {
                ...
                if (_iSessionHandle.localUser.publishState != PublishState_BOTH) {
                    ...
                    //                    [_iSessionHandle.roomMgr publishVideo:nil];
                    //
                    //                    [_iSessionHandle.roomMgr publishAudio:nil];
                }
            }
            ...
            bool canDraw = isStdAndRoomOne?[TKEduSessionHandle shareInstance].iIsCanDrawInit:tIsTeacherOrAssis;
            // 涂鸦权限:1.1v1学生根据配置项设置 2.其他情况，没有涂鸦权限 3 非老师断线重连不可涂鸦。 发送:1 1v1 学生发送 2 学生发送，老师不发送
            [[TKEduSessionHandle shareInstance]configureDraw:canDraw isSend:isStdAndRoomOne?YES:!tIsTeacherOrAssis to:sTellAll peerID:_iSessionHandle.localUser.peerID];
            
            //如果是学生需要重新设置翻页
            [[TKEduSessionHandle shareInstance]configurePage:[TKEduSessionHandle shareInstance].iIsCanDrawInit?true:[TKEduSessionHandle shareInstance].iIsCanPageInit isSend:NO to:sTellAll peerID:isStdAndRoomOne?_iSessionHandle.localUser.peerID:@""];
            ...
            //将所有拖拽的视频还原
            for (TKOldVideoSmallView *view  in self.iStudentVideoViewArray) {
        ...
        if (_iUserType == UserType_Student) [self boardOnFullScreen: isFull];
...
//        videoOriginSuperView = videoOriginSuperView ? videoOriginSuperView : _iRightView;
        [_iRightView addSubview: _iTeacherVideoView];
...
{
    TKOldVideoSmallView * currentBtn = (TKOldVideoSmallView *)longGes.view;
...
-(void)sendMoveVideo:(NSDictionary *)aPlayVideoViewDic aSuperFrame:(CGRect)aSuperFrame allowStudentSendDrag:(BOOL)isSendDrag{
    ...
    for (NSString *tKey in aPlayVideoViewDic) {
        ...
        TKOldVideoSmallView *tVideoView = [aPlayVideoViewDic objectForKey:tKey];
...
-(void)moveVideo:(NSDictionary *)aMvVideoDic{
    ...
    for (NSString *peerId in aMvVideoDic) {
        ...
        TKOldVideoSmallView *tVideoView = [_iPlayVideoViewDic objectForKey:peerId];
...
- (void)sScaleVideo:(NSDictionary *)peerIdToScaleDic{
    ...
    for (NSString *peerId in peerIdArray) {
        ...
        TKOldVideoSmallView *videoView = [self videoViewForPeerId:peerId];
...
- (void)sVideoSplitScreen:(NSMutableArray *)array{
    ...
    for (TKOldVideoSmallView *videoView in vArray) {
...
-(void)onClassTimer {
    ...
    if (!_iSessionHandle.isPlayback) {
        [self.titleView setTime:_iLocalTime];
        //        [self.timeView setClassTime:_iLocalTime];
    }
...
#pragma mark Public
- (TKOldVideoSmallView *)videoViewForPeerId:(NSString *)peerId {
    if (peerId == nil) {
        return nil;
    }
...
- (void)quitClearData {
    ...
    //将分屏的数据删除
    for (TKOldVideoSmallView *view in _iStudentSplitViewArray) {
        //[view clearVideoData];
        [self clearVideoViewData:view];
    }
    [_splitScreenView deleteAllVideoSmallView];
