...
@interface TKOldVideoSmallView ()<CAAnimationDelegate,TKVideoPopupMenuDelegate>
{
    NSString *_giftWav;//记录声音地址
}
@property (nonatomic, strong) TKBackGroundView *sIsInBackGroundView;//进入后台覆盖视图

@property (nonatomic, strong) TKOldMaskView *maskView;//上层背景贴图
...
@implementation TKOldVideoSmallView
...
//        self.sakura.backgroundColor(ThemeKP(@"videoBackColor"));
        ...
        _maskView = [[TKOldMaskView alloc]initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height) aVideoRole:aVideoRole];
