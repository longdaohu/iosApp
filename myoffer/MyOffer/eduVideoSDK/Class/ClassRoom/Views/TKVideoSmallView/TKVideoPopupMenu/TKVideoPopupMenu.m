                            ...
                            delegate:(id<TKVideoPopupMenuDelegate>)delegate
{
    ...
    //1v1&&允许助教上台  ||  原结构老师
    if (([TKEduSessionHandle shareInstance].iRoomProperties.iRoomType == RoomType_OneToOne &&  ![[TKEduSessionHandle shareInstance].roomMgr getRoomConfigration].assistantCanPublish)  || ([tplId isEqualToString:TKClassicTPL] && [skinId isEqualToString:TKOriginSkin] && videoRole == UserType_Teacher)) {
...
- (CGFloat)returnControlButtonCountVideoRole:(EVideoRole)aVideoRole aRoomUser:(TKRoomUser *)aRoomUer isSplit:(BOOL)isSplit{
    ...
    if (aVideoRole == EVideoRoleTeacher || (aVideoRole != EVideoRoleTeacher && [aRoomUer.peerID isEqualToString:[TKEduSessionHandle shareInstance].localUser.peerID])) {
        ...
        if ([[TKEduSessionHandle shareInstance].roomMgr getRoomConfigration].allowStudentCloseAV && aRoomUer.role == UserType_Student) {
            tPoroFloat = 2.0;
    ...
    //如果是1v1课堂减少分屏按钮
//    if ( [TKEduSessionHandle shareInstance].iRoomProperties.iRoomType == RoomType_OneToOne && (aRoomUer.role == UserType_Teacher || aRoomUer.role == UserType_Student) && [[TKEduSessionHandle shareInstance] localUser].role == EVideoRoleTeacher) {
//        tPoroFloat = tPoroFloat - 1;
//    }
    
    //如果是1v1课堂，开始上课，并且是老师角色 则减少一个分屏按钮
    if ([TKEduSessionHandle shareInstance].iRoomProperties.iRoomType == RoomType_OneToOne && [TKEduSessionHandle shareInstance].isClassBegin && [[TKEduSessionHandle shareInstance] localUser].role == EVideoRoleTeacher) {
        tPoroFloat = tPoroFloat -1;
    }
    
    if ([TKEduSessionHandle shareInstance].iRoomProperties.iRoomType == RoomType_OneToOne && aRoomUer.role == UserType_Student && [TKEduSessionHandle shareInstance].localUser.role == UserType_Teacher) {
        tPoroFloat = tPoroFloat -1;
    }
    ...
    if ([tplId isEqualToString:TKClassicTPL] && [skinId isEqualToString:TKOriginSkin] && aVideoRole == EVideoRoleTeacher && [TKEduSessionHandle shareInstance].iRoomProperties.iRoomType != RoomType_OneToOne) {//经典
        
        tPoroFloat = tPoroFloat -1;
    }
   //如果是一对一课堂，并且允许助教上台
    if ([TKEduSessionHandle shareInstance].iRoomProperties.iRoomType == RoomType_OneToOne && [[TKEduSessionHandle shareInstance].roomMgr getRoomConfigration].assistantCanPublish) {
        tPoroFloat = tPoroFloat +1;
    }
    //如果是一对一课堂，并且允许助教上台,并且用户角色是学生
    if ([TKEduSessionHandle shareInstance].iRoomProperties.iRoomType == RoomType_OneToOne && [[TKEduSessionHandle shareInstance].roomMgr getRoomConfigration].assistantCanPublish && aVideoRole == UserType_Student) {
        tPoroFloat = tPoroFloat +1;
    }
    //如果是一对一课堂，并且用户角色是老师，并且为开始上课，原结构模式下
    if (([TKEduSessionHandle shareInstance].iRoomProperties.iRoomType != RoomType_OneToOne && aVideoRole == UserType_Teacher && ![TKEduSessionHandle shareInstance].isClassBegin) && [tplId isEqualToString:TKClassicTPL] && [skinId isEqualToString:TKOriginSkin] ) {
        tPoroFloat = tPoroFloat +1;
    }
    return tPoroFloat;
}
...
- (void)setDefaultSettings
{
    ...
    _point = CGPointZero;
    _borderWidth = 1.0; //设置边框宽度
    _borderColor = [TKTheme colorWithPath:ThemeKP(@"videoPopBorderColor")];//设置边框颜色
    
    _arrowWidth = 15.0;
    _arrowHeight = 10.0;
    _backColor = [TKTheme colorWithPath:ThemeKP(@"videoPopBackColor")];//设置背景颜色
...
-(UIView *)videoToolView{
    ...
    if (!_videoToolView) {
        ...
        if (([TKEduSessionHandle shareInstance].iRoomProperties.iRoomType == RoomType_OneToOne && ![[TKEduSessionHandle shareInstance].roomMgr getRoomConfigration].assistantCanPublish)|| ([tplId isEqualToString:TKClassicTPL] && [skinId isEqualToString:TKOriginSkin] && _videoRole == UserType_Teacher )) {
...
- (void)updateUI
{
    ...
    if (_arrowDirection == TKPopupMenuArrowDirectionTop) {
        ...
        if (_videoRole == EVideoRoleOur && IS_IPHONE && _controlButtonCount>3) {
