...
-(instancetype)initWithFrame:(CGRect)frame aVideoRole:(EVideoRole)aVideoRole aRoomUer:(TKRoomUser*)aRoomUer isSplit:(BOOL)isSplit count:(CGFloat)count{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _iButton1 = ({//涂鸦授权
            ...
            TKButton *tButton = [self returninitFrame:CGRectMake(10, 0, tWidth, tHeight) buttonWidth:tWidth tHeight:tHeight imageName:ThemeKP(@"videoPopGraffitiNomalImage") selectImageName:ThemeKP(@"videoPopGraffitiSelectedImage") title:MTLocalized(@"Button.AllowDoodle") selectTitle:MTLocalized(@"Button.CancelDoodle") action:@selector(button1Clicked:) selected:aRoomUer.canDraw];
        ...
        _iButton2 = ({//上下台控制
            ...
            TKButton *tButton = [self returninitFrame:CGRectMake(10+tWidth, 0, tWidth, tHeight) buttonWidth:tWidth tHeight:tHeight imageName:ThemeKP(@"icon_control_up") selectImageName:ThemeKP(@"icon_control_down") title:MTLocalized(@"Button.UpPlatform") selectTitle:MTLocalized(@"Button.DownPlatform") action:@selector(button2Clicked:) selected:(aRoomUer.publishState != PublishState_NONE)];
        ...
        if (aRoomUer.disableVideo == NO) {
            _iButton5 = ({//关闭视频
                ...
                TKButton *tButton = [self returninitFrame:CGRectMake(10+tWidth*3, 0, tWidth, tHeight) buttonWidth:tWidth tHeight:tHeight imageName:ThemeKP(@"icon_control_camera_01") selectImageName:ThemeKP(@"icon_control_camera_02") title:MTLocalized(@"Button.OpenVideo") selectTitle:MTLocalized(@"Button.CloseVideo") action:@selector(button5Clicked:) selected:isSelected];
...
//        ============ 老师 ===============
        if (aVideoRole == EVideoRoleTeacher) {
            ...
            _iButton1 = ({//关闭视频
                ...
                TKButton *tButton = [self returninitFrame:CGRectMake(10, 0, tWidth, tHeight) buttonWidth:tWidth tHeight:tHeight imageName:ThemeKP(@"icon_control_camera_01") selectImageName:ThemeKP(@"icon_control_camera_02") title:MTLocalized(@"Button.OpenVideo") selectTitle:MTLocalized(@"Button.CloseVideo") action:@selector(button1Clicked:) selected:isSelected];
                
                tButton;
                
            });
            _iButton2 = ({//关闭音频
                BOOL isSelected = !(aRoomUer.publishState == PublishState_AUDIOONLY || aRoomUer.publishState == PublishState_BOTH);
...
//            [self addSubview:_iButton6];
            ...
            _iButton3.frame = CGRectMake(tWidth, 0, tWidth, tHeight);
            _iButton5.frame = CGRectMake(2*tWidth, 0, tWidth, tHeight);
            _iButton6.frame = CGRectMake(3*tWidth, 0, tWidth, tHeight);
            [self addSubview:_iButton2];
            [self addSubview:_iButton3];
            [self addSubview:_iButton5];
            [self addSubview:_iButton6];
            ...
            if (!isSplit) {
                ...
                _iButton3.frame = CGRectMake(tWidth, 0, tWidth, tHeight);
                _iButton5.frame = CGRectMake(2*tWidth, 0, tWidth, tHeight);
                _iButton4.frame = CGRectMake(3*tWidth, 0, tWidth, tHeight);
                _iButton6.frame = CGRectMake(4*tWidth, 0, tWidth, tHeight);
            }
            [self addSubview:_iButton2];
            [self addSubview:_iButton3];
            [self addSubview:_iButton5];
