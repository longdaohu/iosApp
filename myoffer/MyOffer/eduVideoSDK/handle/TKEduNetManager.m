...
+ (void)downLoadTaskToSandboxWithHost:(NSString *_Nonnull)host taskDic:(NSDictionary *_Nonnull)taskDic complete:(bComplete _Nonnull )aComplete aNetError:(bError _Nullable) aNetError
{
    ...
    for (NSString *key in taskDic.allKeys) {
        NSArray *arr = [taskDic[key] componentsSeparatedByString:@"."];
        if (arr && arr.count == 2) {
            [downloadArr addObject:key];
        }
    }
    ...
    NSString *pathTrophy = [pathDir stringByAppendingPathComponent:[NSString stringWithFormat:@"trophyname%@", taskDic[@"trophyname"]]];
    
    [[NSFileManager defaultManager] createDirectoryAtPath:pathTrophy withIntermediateDirectories:YES attributes:nil error:&error];
    
    if (!error) {
        for (NSString *keyname in downloadArr) {
            NSString *url = [NSString stringWithFormat:@"%@%@", host, taskDic[keyname]];
            NSString *filePath = [pathTrophy stringByAppendingPathComponent:keyname];
...
- (void)downloadTaskToSandboxWithUrlStr:(NSString *)urlStr fliePath:(NSString *)filePath
{
    TKAFHTTPSessionManager *manager = [TKAFHTTPSessionManager manager];
