...
//    NSLog(@"xLabels %@  num %lf   _xLabelWidth %lf  boundW  %lf",xLabels,num,_xLabelWidth,self.frame.size.width);

    //坐标数字
    for (int i=0; i<xLabels.count; i++) {
        NSString *labelText = xLabels[i];
        UUChartLabel * label = [[UUChartLabel alloc] initWithFrame:CGRectMake(i * _xLabelWidth+UUYLabelwidth, self.frame.size.height - UULabelHeight - 10, _xLabelWidth, UULabelHeight)];
...
-(void)strokeChart
{
    for (int i=0; i<_yValues.count; i++) {
        ...
        if ([[_colors objectAtIndex:i] CGColor]) {
            _chartLine.strokeColor = [UIColor lightGrayColor].CGColor; //[[_colors objectAtIndex:i] CGColor];
        }else{
            _chartLine.strokeColor = [UIColor lightGrayColor].CGColor;
        }
...
//        view.backgroundColor = [_colors objectAtIndex:index]?[_colors objectAtIndex:index]:[UUColor green];
        ...
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point.x-UUTagLabelwidth/2.0, point.y-chartMargin*2  , UUTagLabelwidth, UULabelHeight)];
        label.font = [UIFont systemFontOfSize:15];
        label.textAlignment = NSTextAlignmentCenter;
        label.textColor = XCOLOR_LIGHTBLUE;
