...
static CGFloat const weeksH = 50;       //周高度
...
- (void)dealloc
{
    KDClassLog(@"日历 + YXCalendarView + dealloc");
...
- (void)settingHeadLabel {
    ...
    _yearMonthL.font = [UIFont boldSystemFontOfSize:15];
    [self addSubview:_yearMonthL];
    
    UIButton *preBtn = [[UIButton alloc] initWithFrame:CGRectMake(ViewW * 0.5 - 100, 0, 30, yearMonthH)];
    [self addSubview:preBtn];
    [preBtn addTarget:self action:@selector(lastWeek) forControlEvents:UIControlEventTouchUpInside];
    [preBtn setImage:XImage(@"back_arrow_black") forState:UIControlStateNormal];
    
    UIButton *nextBtn = [[UIButton alloc] initWithFrame:CGRectMake(ViewW * 0.5 + 70 , 0, 30, yearMonthH)];
    [self addSubview:nextBtn];
    [nextBtn addTarget:self action:@selector(nextWeek) forControlEvents:UIControlEventTouchUpInside];
    [nextBtn setImage:XImage(@"next_arrow_black") forState:UIControlStateNormal];
