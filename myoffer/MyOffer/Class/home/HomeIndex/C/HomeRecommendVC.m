...
- (void)caseInvitationActivityWithResponse:(id)response{
    ...
    NSDictionary *result = response[@"result"];
    NSString *path = [NSString stringWithFormat:@"https://m.myoffer.cn/invitation-activity.html?rewarded=%@&waitAward=%@&total=%@&accountId=%@",result[@"rewarded"],result[@"waitAward"],result[@"total"],[[MyofferUser defaultUser] user_id]];
