...
- (void)initWithProperty
{
    self.itemMargin = 20;
    self.selectIndex = 0;
    self.titleNormalColor = XCOLOR_TITLE;
    self.titleSelectColor = XCOLOR_LIGHTBLUE;
...
- (void)moveIndicatorView:(BOOL)animated
{
    ...
    [UIView animateWithDuration:(animated?ANIMATION_DUATION:0) animations:^{
        ...
        self.indicatorViewFrame = self.indicatorView.frame;
...
- (void)setProgress:(CGFloat)progress{
    ...
    CGRect rect = self.indicatorViewFrame;

    if (progress <= 1) {
        
        rect.size.width =  rect.size.width * (1 + progress);
